    ...
    public void run() {
        ...
        if (threadName.equals("Student1")) {
            ...
            try {
                Thread.sleep(4000);
                synchronized (res3) {
                    System.out.println("Student 2 has acquired resource 1 " + res3);
                    Thread.sleep(4000);
                    synchronized (res2) {
                        System.out.println("Student 2 has acquired resource 2 " + res2);
                        Thread.sleep(4000);
                        synchronized (res1) {
                            System.out.println("Student 2 has acquired resource 3 " + res1);
                            Thread.sleep(5000);
                        }
                    }
                }
...
/* output : -> deadlock created
Student 2 has acquired resource 1 MERN book
